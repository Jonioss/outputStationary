...
void gemm2(const hls::vector<fm_t, 16> A_DRAM[I][K/16],const hls::vector<fm_t, 16> B_DRAM[K/16][J], hls::vector<fm_t, 16> C_DRAM[I][J/16]) {
	...
	#pragma HLS ARRAY_PARTITION variable=A_BUF dim=1 type=complete
	...
	#pragma HLS stream variable=B_BUF type=pipo depth=2
	...
	#pragma HLS stream variable=C_BUF type=pipo depth=2
